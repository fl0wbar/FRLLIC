...
def print_join(it, joiner="\n"):
    """
    Like print(joiner.join(it)) but for iterators that take a long time 
    to produce elements, this version prints elements as they are available
    """
    it = iter(it)
    first = next(it)
...
class ProgressPrinter(object):
    ...
    def __init__(self, info=None, iter_list=None):
        ...
        """
        assert iter_list is None or isinstance(
            iter_list, collections.Sequence
        ), "iter_list must be sequence"
        if info:
            print(info)
...
def progress_print(p, _last_pstr_cache=[""], _reset_cache=False):
    ...
    ):  # prevents flickering on some consoles: only update if something changed!
        _last_pstr_cache[0] = pstr
