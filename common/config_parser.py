...
_SUB_SEP = os.environ.get("common_CONFIGP_SUBSEP", ".")
...
def parse(config_p):
    ...
    rel_path = (
        path.abspath(config_p).replace(path.dirname(root_path), "").strip(path.sep)
    )
...
def _parse(config_p):
    with open(config_p, "r") as f:
        ...
        if len(lines) == 0:
            raise ValueError(
                "Invalid config file, not enough lines: {}".format(config_p)
            )
...
def _update_config(config, lines):
    for line in _merge_multiline_statements(lines):
        ...
        globals_dict = dict(
            config.__dict__, **{val: val for val in config.all_constraint_values()}
        )
        try:
            var_value = eval(
                var_value, globals_dict
            )  # pass current config as globals dict
        except SyntaxError:
...
class _Config(object):  # placeholder object filled with setattr
    ...
    def add_constraint(self, var_name, allowed_var_values):
        if var_name in self._constraints:
            raise ValueError(
                "Already have constraint for {}, not overwriting!".format(var_name)
            )
    ...
    def assert_fullfills_constraint(self, var_name, var_value):
        ...
        if var_value not in allowed_var_values:
            raise ConstraintViolationException(
                "{} does not fullfill constraint {} :: {}".format(
    ...
    def all_params_and_values(self):
        return (
            (k, v) for k, v in sorted(self.__dict__.items()) if self.is_valid_key(k)
        )
    ...
    def __getattr__(self, item):
        ...
        if len(filtered_dict) == 0:
            raise AttributeError(
                "{} has no attribute {} and no attributes starting with {}".format(
